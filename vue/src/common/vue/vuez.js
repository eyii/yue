...
Vue.component('einput',{
    template: '<el-col ' +
        ':span="span" :offset="offset"><el-input style="width:250px;" v-model="result" :size="size" :type="type"><template slot="prepend">{{title}}</template> </el-input></el-col>',
    props:{
        title:'',
        result:'',
        type:{default:'',},
...
Vue.component('ebutton', {
    template: '<el-col ' +
        ':span="span" :offset="offset"><el-button  :type="type" v-on:click="click">{{ title }}</el-button></el-col>',
    props: {
        title:'',
        span: {
            default:2,
...
Vue.component('currency-input', { template:'<span>' +
        '<input v-on:input="$emit(\'input\', $event.target.value)"></span>', });
Vue.component('ebtn', {template: '<el-button ' +
        ':size="size" :type="type" v-on:click="$emit(\'click\')"  >{{ title }}</el-button>',
    props: {
        title:'',
        size: {
            default:"mini",
...
Vue.component('edialog',{template:'<el-dialog ' +
        ' :visible.sync="show"  :width="width"  :before-close="handleClose" ><slot></slot></el-dialog>',
    props: {
        width:{ default:'30%' },
        show:{ default:false }
    },
...
Vue.component('epage',{
    template:'<el-row>' +
        '<el-col><div class="block"><el-pagination @size-change="size" @current-change="current" :current-page="page" :page-size="pageSize"    layout="total, sizes, prev, pager, next, jumper" :total="param.total"></el-pagination></div></el-col></el-row> ',
    props: {
...
Vue.component('estatus',{
    template:'<el-table-column ' +
        ':label="title"><template slot-scope="scope"><el-switch v-model="status" active-value="1"/></template></el-table-column>',
    props: {
        title:{
            default:'状态',
        },
...
Vue.component('ecolop',{
    template: ' <el-table-column>' +
        '<template slot-scope="scope"><el-button v-on:click="add"  >{{ title }}</el-button> </template> </el-table-column>',
    props: {
        title:'',
    },
    methods: {
...
Vue.component('ealert',{
    template: '<el-col ' +
        ':span="span"><el-alert :title="title" :type="type" :description="description" ></el-col>',
    props:{
...
Vue.component('mcheckbox',{
    template:'<el-col ' +
        ':span="span" :offset="offset"  >  <input :type="type" :id="id" :value="value" :name="name" v-model="result" @change="change" :checked="checked" />  <label :for="id">{{title}}</label></el-col>',
    props:{
        id:'',
        title:'',
        result:'',
...
//  url,param,success,error="操作失败",catchCallback
...
Vue.component('eop',{
    template:'<el-table-column ' +
        'label="操作" width="420"> <template slot-scope="scope"><slot></slot></template></el-table-column>',
});
